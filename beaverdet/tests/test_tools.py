...
def test_window_sympy_solver():
   ...
   Conditions tested:
    ...
    with pytest.raises(ValueError, match='Bad keyword argument:\npulled_pork'):
        tools.window_sympy_solver(**kwargs)
    ...
    with pytest.warns(Warning,
        ...
        test_nan = tools.window_sympy_solver(**kwargs)
    ...
    for index in range(len(args)):
        test_output = tools.window_sympy_solver(**args[index])
...
def test_import_thread_specs():
    ...
    specifications = tools.import_thread_specs()
...
def test_get_thread_property():
    ...
    """
    dataframes = tools.import_thread_specs()
    ...
    for args, result in zip(good_args, good_results):
        test_result = tools.get_thread_property(*args)
